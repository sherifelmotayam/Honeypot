...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 13 17:05:40 2022
...
class SQLInjectionDetector:
    ...
    def __init__(self):
        ...
        print("Waiting for SQL Injection Detector Machine Learning Preprocessing")
        # print (dataframe['type'].value_counts())

        # tokenize raw sql
        dataframe['sql_tokens'] = dataframe['raw_sql'].map(lambda x: self.__Sql_tokenizer(x))

        
        # get token sequences
        dataframe['token_seq'] = dataframe['sql_tokens'].map(lambda x: self.__GetTokenSeq(x, 3))
        ...
        self.__tc_dataframe = self.__G_test(pd.Series(_tokens), pd.Series(_types))

        # now we set real features for machine learning algorithm.
        dataframe['token_length'] = dataframe['sql_tokens'].map(lambda x: len(x))
        dataframe['entropy'] = dataframe['raw_sql'].map(lambda x: self.__Entropy(x))
        dataframe['sqli_g_means'] = dataframe['token_seq'].map(lambda x: self.__G_means(x, 'sqli_GTest'))
        dataframe['plain_g_means'] = dataframe['token_seq'].map(lambda x: self.__G_means(x, 'plain_GTest'))
        ...
        self.__clf = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=7, random_state=0).fit(X_train, y_train)
        print ("Gradient Boosting Tree Acurracy: {}%".format(self.__clf.score(X_test, y_test)))
    ...
    def __Sql_tokenizer(self, raw_sql):
        if self.__sql_regex.search(raw_sql):
            return [tok[0] for tok in groupby([match.lastgroup for match in self.__sql_regex.finditer(raw_sql)])]
        else:
            return ['PLAIN']

    def __GetTokenSeq(self, token_list, N):
        token_seq = []
        for n in range(0,N):
    ...
    def __G_test_score(self, count, expected):
        if (count == 0):
            return 0
        else:
    ...
    def __G_test(self, tokens, types):
        ...
        for column in tc_dataframe.columns.tolist():
            ...
            tc_dataframe[column+'_GTest'] = [self.__G_test_score(tkn_count, exp) for tkn_count, exp in zip(tc_dataframe[column], tc_dataframe[column+'_exp'])]

        return tc_dataframe

    # get string entropy for use another feature.
    # but this feature is not enough to me.
    def __Entropy(self, raw_sql):
        p, lns = collections.Counter(str(raw_sql)), float(len(str(raw_sql)))
        return -sum( count/lns * math.log(count/lns, 2) for count in p.values())

    # get g-score means of each tokens
    def __G_means(self, token_seq, c_name):
        try:
            g_scores = [self.__tc_dataframe.loc[token][c_name] for token in token_seq]
        except KeyError:
            return 0
        return sum(g_scores)/len(g_scores) if g_scores else 0 # Average

    # you can check your test data.
    def Check_is_sql(self, sql):
        # do some pre-processing remoce comment /**/, /*!num */
        _tmp = re.sub(r'(/\*[\w\d(\`|\~|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\=|\+|\[|\{|\]|\}|\\|\:|\;|\'|\"|\<|\>|\,|\.|\?)\s\r\n\v\f]*\*/)', ' ', sql)
        _tmp = re.sub(r'(/\*!\d+|\*/)', ' ', _tmp)

        sql_tokens = self.__Sql_tokenizer(_tmp.strip())
        token_seq = self.__GetTokenSeq(sql_tokens, 3)
        sqli_g_means = self.__G_means(token_seq, 'sqli_GTest')
        plain_g_means = self.__G_means(token_seq, 'plain_GTest')
        _X = [[len(sql_tokens), self.__Entropy(sql), sqli_g_means, plain_g_means]]
        return self.__clf.predict(_X)[0]


# if __name__ == "__main__":

#     x = SQLInjectionDetector()
